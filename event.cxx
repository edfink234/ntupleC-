...
Event::Event(TChain* entry, TChain* event_info_entry, int entry_number)
: run_number{-1}, random_run_number{-1}, event_number{-1}, entry_number{entry_number}
{
...
//    static int count = 0;
    ...
    if (cache_truth)
    {
        ...
        __load_truth_particles(entry,entry_number);
    }
    if (load_reco)
    {
        if (load_photons)
        {
            ...
            __load_photons(entry,entry_number);
        }
        if (load_electrons)
        {
            __load_electrons(entry,entry_number);
        }
        if (load_clusters)
        {
            __load_clusters(entry,entry_number);
        }
        if (load_tracks)
        {
            __load_tracks(entry,entry_number);
        }
        if (load_triggers)
        {
            __load_triggers(entry,entry_number);
        }
    }
//    ++count;
...
//    printf("hi\n");
    ...
    entry_number = other.entry_number;
...
//
    ...
    for (decltype(length) i = 0; i < length; ++i)
    {
        ...
        electrons.emplace_back(Electron(entry, i, pt, energy, entry_number));
    }

}

void Event::__load_clusters(TChain* entry, int entry_number)
{
    std::vector<double> *cluster_pt= nullptr;
    entry->SetBranchAddress("cluster_pt",&cluster_pt);
    
    entry->GetBranch("cluster_pt")->GetEntry(entry_number);
    
    auto length = (*cluster_pt).size();
    
    for (decltype(length) i = 0; i < length; ++i)
    {
        clusters.emplace_back(Cluster(entry, i, entry_number));
    }
}

void Event::__load_tracks(TChain* entry, int entry_number)
{
    std::vector<double> *track_pt= nullptr;
    entry->SetBranchAddress("track_pt",&track_pt);
    
    entry->GetBranch("track_pt")->GetEntry(entry_number);
    ...
    for (decltype(length) i = 0; i < length; ++i)
    {
        ...
        if (!entry->GetListOfBranches()->FindObject("track_type"))
        {
            tracks.emplace_back(Track(entry,i,entry_number));
        }
        // FIXME: 
        else
        {
            pixel_tracks.emplace_back(Track(entry,i,entry_number));
        }
        
    }
}

void Event::__load_triggers(TChain* entry, int entry_number)
{
//    puts("__load_triggers");
    if (entry->GetListOfBranches()->FindObject("trigger_passed_triggers"))
    {
       ...
       entry->GetBranch("trigger_passed_triggers")->GetEntry(entry_number);
...
void Event::__load_truth_particles(TChain* entry, int entry_number)
{
    std::vector<double> *mc_pt= nullptr;
    entry->SetBranchAddress("mc_pt",&mc_pt);
    
    entry->GetBranch("mc_pt")->GetEntry(entry_number);
    
    auto length = (*mc_pt).size();
    
    for (decltype(length) i = 0; i < length; ++i)
    {
        truth_particles.emplace_back(TruthParticle(entry, i, entry_number));
    }
}
      ...
      int* status_code)
{
    ...
    else
    {
        ...
        entry.GetBranch("mc_pdg_id")->GetEntry(entry_number);
        ...
        for (decltype(length) i = 0; i < length; ++i)
        {
            if (!barcode.empty())
            {
                ...
                entry.GetBranch("mc_barcode")->GetEntry(entry_number);
            ...
            if (!parent_barcode.empty())
            {
                ...
                entry.GetBranch("mc_parent_barcode")->GetEntry(entry_number);
            ...
            if (status_code)
            {
                ...
                entry.GetBranch("mc_status")->GetEntry(entry_number);
            ...
            results.push_back(TruthParticle(&entry,i, entry_number));
